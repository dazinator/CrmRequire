...
namespace CrmRequire.Requirements
{
    ...
    {
        public SolutionMinimumVersionRequirement(SolutionExistsRequirement requirment, string versionNumber)
        {
            this.VersionNumber = versionNumber;
            this.SolutionExistsRequirement = requirment;
        }
            ...
            using (var service = crmServiceProvider.GetOrganisationService() as OrganizationServiceContext)
            {
             ...
             //   var versions = new List<KeyValuePair<int, Entity>>();
              //  var semversions = solutions.Select(a => new { Entity = a, Version = SemVersion.Parse((string)a["versionnumber"]) });
                ...
                {
                    failureMessage = "The min supported solution version number has not been met.";
                    Fail(failureMessage);
                    return false;
                }
                else
                {
                    Success("Latest solution is greater than min version.");
